...
final class StaticFixtureSplitter
{
	...
	public static function createTemporaryFileInfo(SmartFileInfo $fixtureSmartFileInfo, string $prefix, string $fileContent, bool $preserveName = false): SmartFileInfo
	{
		$temporaryFilePath = self::createTemporaryPathWithPrefix($fixtureSmartFileInfo, $prefix, !$preserveName);
	...
	public static function splitFileInfoToLocalInputAndExpectedFileInfos(SmartFileInfo $smartFileInfo, bool $autoloadTestFixture = \false, bool $preserveDirStructure = \true): InputFileInfoAndExpectedFileInfo
	{
		$inputAndExpected = self::splitFileInfoToInputAndExpected($smartFileInfo);
		$prefix           = '';
		...
		if ($preserveDirStructure)
		{
			...
			$inputFileInfo = self::createTemporaryFileInfo($smartFileInfo, $prefix, $inputAndExpected->getInput(), true);
		}
		else
		{
	...
	private static function createTemporaryPathWithPrefix(SmartFileInfo $smartFileInfo, string $prefix, bool $withHash = true): string
	{
		...
		return self::getTemporaryPath() . \sprintf('/%s%s', $prefix, $fileBasename);
	}
